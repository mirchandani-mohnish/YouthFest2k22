...
class Resume extends Component {
  ...
  async componentDidMount() {
    try {
      const eventList = await Axios.get("http://157.245.98.117:8000/events");
    ...
    return (
      <section id="resume">
        ...
        {this.state.EventPg === true ? (
          <Modal
            currEvent={this.state.CurrentEventData}
          ...
          <div>
            <div className="row education">
              ...
              <div className="eleven columns main-col">
                <div className="cards-list">{this.renderEvents()}</div>
              </div>
            </div>
          </div>
        )}
